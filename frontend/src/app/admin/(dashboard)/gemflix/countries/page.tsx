    ...
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-400">Gestiona los países de producción de las películas y series.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/5 border-b border-white/5">
                ...
                <th className="py-4 px-6 text-xs font-semibold text-gray-400 uppercase tracking-wider">ISO 3166-1</th>
            ...
            <tbody className="divide-y divide-white/5">
              {isLoading ? (
                ...
                <tr key={country.id} className="hover:bg-white/2 transition-colors group">
                  <td className="py-4 px-6 text-sm text-gray-400">#{country.id}</td>
                  <td className="py-4 px-6">
                    <p className="text-white font-medium">{country.name || '-'}</p>
                  </td>
                  <td className="py-4 px-6">
                    <p className="text-gray-400 font-medium">{country.english_name || '-'}</p>
                  </td>
                  <td className="py-4 px-6">
                    <span className="bg-white/10 text-white px-2 py-1 rounded text-xs uppercase">{country.iso_3166_1 || '-'}</span>
                  </td>
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2">
                      <button 
                        onClick={() => { setEditingItem(country); setModalMode("edit"); }}
              ...
              {!isLoading && countries.length === 0 && (
                <tr><td colSpan={5} className="py-8 text-center text-gray-400">No hay países registrados.</td></tr>
              )}
            </tbody>
          </table>
        </div>
